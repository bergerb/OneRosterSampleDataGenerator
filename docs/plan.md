...
## Approach
- Add `OneRosterVersion` enum (`V1_1`, `V1_2`) exposed on `Args`
- Default to `V1_1` for backward compatibility
- **Separate export DTOs per version** organized by namespace/folder:
  - `Models/Exports/` — v1.1 (existing, with bug fixes)
  - `Models/Exports/V12/` — v1.2 (new)
- Domain models (`User`, `Enrollment`, etc.) remain mostly version-agnostic; only CSV column schemas differ
...
|------|-------------|---------|
| `roles.csv` | User→Org→Role mapping (replaces `orgSourcedIds`/`role` in users.csv) | `absent` |
...
**Fixes vs current**: `nativeAmericanOrOtherPacificIslander` → `nativeHawaiianOrOtherPacificIslander`; Added `white`, `demographicRaceTwoOrMoreRaces`, `hispanicOrLatinoEthnicity`
...
| `Models/Exports/V12/DemographicFile.cs` | v1.2 demographic CSV: fixed column names, added columns |
| `Models/Exports/V12/ClassFile.cs` | v1.2: `termSourcedIds`, `subjectCodes` |
| `Models/Exports/V12/EnrollmentFile.cs` | v1.2: add `courseSourcedId` |
...
| File | Changes |
|------|---------|
| `Models/Vocab.cs` | Add `OneRosterVersion` enum (`V1_1`, `V1_2`); expand `Gender` for v1.2 |
| `OneRoster.cs` | Add `Version` to `Args` (default `V1_1`); pass to `FileProcessor` and `Manifests`; conditionally call `Roles` generator + output `roles.csv`; update `OutputCSVFiles()` for version-aware routing; update `GetCurrentFileStats()` |
...
| `Models/User.cs` | Implement new interface members |
| `FileProcessor.cs` | Accept `OneRosterVersion` param; route to V11 or V12 export DTOs; add new type mappings for `Role` |
| `Models/Manifests.cs` | Accept `OneRosterVersion`; emit `"1.2"` + 8 new file entries for v1.2; emit `"1.1"` + current entries for v1.1 |
| `Models/Exports/DemographicFile.cs` | **BUG FIX**: Fix `nativeAmericanOrOtherPacificIslander` → `nativeHawaiianOrOtherPacificIslander`; add missing `white`, `demographicRaceTwoOrMoreRaces`, `hispanicOrLatinoEthnicity` |
| `Models/Exports/ClassFile.cs` | **BUG FIX**: Fix `SubjectCodes` → `subjectCodes` (lowercase) |
| `Models/Exports/EnrollmentFile.cs` | **BUG FIX**: Add `courseSourcedId` column |
| `Consts.cs` | Add `RolesFile`, `AcademicSessionsFile`, `DemographicsFile` constants if missing; update `OneRosterFiles` for v1.2 |
...
3. **BUG FIX**: Fix `DemographicFile.cs` column name + add missing columns
4. **BUG FIX**: Fix `ClassFile.cs` `SubjectCodes` → `subjectCodes`
5. **BUG FIX**: Fix `EnrollmentFile.cs` add `courseSourcedId`
6. Create `Role.cs` + `Roles.cs` domain/generator
7. Create `Models/Exports/V12/` folder with all v1.2 export DTOs
8. Update `FileProcessor.cs` for version-aware routing
9. Update `Manifests.cs` for versioned output
10. Update `OneRoster.cs` for v1.2 pipeline (add `Version` to `Args`, roles generation + output)
11. Update `Consts.cs`
12. Update tests — parameterize over version, add v1.2 coverage
...
- NuGet version: `1.1.0-beta`

## 8. Bug Fixes for v1.1 (existing code — fix regardless of version)

These are bugs in the current v1.1 output that need correction:
1. **DemographicFile.cs**: `nativeAmericanOrOtherPacificIslander` → `nativeHawaiianOrOtherPacificIslander` (wrong column name per spec)
2. **DemographicFile.cs**: Missing columns `white`, `demographicRaceTwoOrMoreRaces`, `hispanicOrLatinoEthnicity` (exist in domain model but not exported)
3. **ClassFile.cs**: `SubjectCodes` → `subjectCodes` (property name should be lowercase)
4. **EnrollmentFile.cs**: Missing `courseSourcedId` column (exists in domain model, used in code, but not exported to CSV)
